  ...
  test(
    ...
    async (t) => {
      ...
      await writeFile(
        ...
        JSON.stringify({ enabled: true, keepRecentTokens }),
      );
      ...
      assert.equal(object(checkpoint.details)["model"], modelId);
      assert.equal(Boolean(object(checkpoint.details)["retained"]), keepRecentTokens > 0);
      ...
      assert.ok(
        afterCheckpoint.some(
          (entry) => entry.type === "custom" && entry.customType === REQUEST_TYPE,
        ),
        "Requests after compaction pass through the extension's provider wrapper.",
      );
      await client.stop();
      t.diagnostic(
        `Pi ${piVersion} loaded the packaged extension for ${modelId}: native compaction, signed replay, and resume passed (keepRecentTokens=${keepRecentTokens}).`,
      );
    },
  );
}
