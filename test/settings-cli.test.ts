...
for (const mode of ["regular", "fullscreen"]) {
  test(`packaged settings menu in ${mode} Pi TUI`, { timeout: 30_000 }, async (t) => {
    const temporary = await mkdtemp(join(tmpdir(), "anthropic-settings-cli-"));
    ...
    terminal.write("/anthropic-settings\r");
    await wait("Draft changes apply");
    terminal.write("Native compaction");
    await setTimeout(100);
    terminal.write("\r");
    await wait("draft change");
    const file = join(agent, "pi-anthropic-compat.json");
    ...
    assert.match(stripVTControlCharacters(output), /Native compaction\s+on ~/);
    terminal.write("\u0013");
